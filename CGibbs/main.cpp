...
// Output file preparation here //
    ...
    std::string prefix = genome_dir; // prefix=results/model/model
    prefix.append(genome_name); prefix.append("/"); prefix.append(genome_name);
    
    std::string out_filename1 = prefix; out_filename1.append(".r");
    ...
    std::string out_filename2 = prefix;
    ...
    std::string postfix3 = ""; std::string postfix4 = ""; std::string postfix5 = ""; std::string postfix6 = ""; std::string postfix7 = "";

// Input file preparation here //
    if (swissprot.compare("1") == 0) { // if argv[3] == 1
        cerr << "here old swprot" << endl;
        postfix3 = ".gene_sw1"; postfix4 = ".ort_sw1"; postfix5 = ".pc.Z_sw1"; postfix6 = ".gc.Z_sw1"; postfix7 = ".pc.Z_sw1";
    }
    else{
        postfix3 = ".gene"; postfix4 = ".ort"; postfix5 = ".pc.Z"; postfix6 = ".gc.Z"; postfix7 = ".pc.Z";
    }
    std::string in_filename1; std::string in_filename2; std::string in_filename3; std::string in_filename4; std::string in_filename5; std::string in_filename6;

    in_filename1 = global_net_file; // database/GRCENEECnet.txt

    // results/$1/$1.gene (gene list & gene-ec homology)
    in_filename2.append(prefix); in_filename2.append(postfix3);

    // results/$1/$1.ort (gene-ec orthology)
    in_filename3.append(prefix); in_filename3.append(postfix4);

    // results/$1/$1.pc.Z (gene-gene phylogenetic corr)
    in_filename4.append(prefix); in_filename4.append(postfix5);

    // results/$1/$1.gc.Z (gene-gene gene clustering corr)
    in_filename5.append(prefix); in_filename5.append(postfix6);

    // results/$1/$1.pc.Z (gene-gene co-expression corr)
    in_filename6.append(prefix); in_filename6.append(postfix7);

    // gene clustering z score to prob. of two being neighbor
    map<int, double> gcZ_Pnb; map_gcZ_Pnb(gcZ_Pnb);

    // phylogenetic correlation z score to prob. of two being neighbor
    map<int, double> phZ_Pnb; map_phZ_Pnb(phZ_Pnb);

    // gene co-expression z score to prob. of two being neighbor
    map<int, double> ceZ_Pnb; map_ceZ_Pnb(ceZ_Pnb);
    //cerr << "map_ceZ_Pnb done" << endl;

    // sequence identity score (0~100) to prob. of being correct assignment
    map<int, double> hmZ_Ptr; map_hmZ_Ptr(hmZ_Ptr);
